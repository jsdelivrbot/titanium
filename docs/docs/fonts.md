...
Font support for Titanium is provided via Cobalt, a JS-based font stack that allows access to the entire Google Fonts library via a single line but only pulls the fonts you require.
...
To use a font, use the class `font-` followed by its name, all-lowercase and with spaces as hyphens. For example, `Annie Use Your Telescope` would become `font-annie-use-your-telescope`.

A full list of fonts followed by their respective class is available below.
...
* Bubblegum Sans `font-bubblegum-sans`
...
* Forum `font-forum`
* Francois One `font-francois-one`
...
* IM Fell English `font-im-fell-english`
...
* Loved by the King `font-loved-by-the-king`
...
* Raleway Dots `font-raleway-dots`
...
* Sedgwick Ave Display `font-sedgwick-ave-display`
